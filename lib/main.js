...
export default {
  ...
  config: {
    uflashExecutable: {
      title: 'uflash executable',
      description: 'For creating the HEX file, and flashing the micropython firmware to ' +
    ...
    pyocdFlashToolExecutable: {
      title: 'pyocd-flashtool executable',
      description: 'An alternative tool for flashing the microbit is `pyocd-flashtool` ' +
                   'from `pyOCD`. To install `pyOCD`, you could use ' +
                   '`sudo pip2 install pyOCD` (note, it needs Python 2.7).',
      type: 'string',
    ...
    flashMethod: {
      title: 'which flash method should be used',
      description: 'Flashing could be done with `uflash` utility or with `pyOCD`.' +
                   '`uflash` uses the USB drive of the microbit for flashing, ' +
                   '`pyOCD` uses a driect DAP-link connection (and thus is in ' +
                   'general more reliable).',
      type: 'string',
      default: 'MICROBIT drive',
      enum: ['MICROBIT drive', 'pyOCD'],
      order: 25
    },
    micropythonFirmware: {
  ...
  flash()
  {
    ...
    if(editor && editor.isModified()) {
      ...
      var source = editor.getPath();
      var flashMethod = atom.config.get('microbit-micropython.flashMethod');

      if(flashMethod == "pyOCD") {
        var target = source.substr(0, source.length - editor.getTitle().length);
        this.pyOCD.createHexAndflash(source, target, this.busySignal);
      }
      else {
